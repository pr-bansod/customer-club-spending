...
def get_agg_features(df, feature_tuple):
    """ to get the aggreated features 

    Arguments:
        df -- dataframe
        feature_tuple {[type]} -- tuple with features
...
def run_XGB(params, train, test, feature_names, n_folds=10, seed=0):
    ...
    for i, (itr, icv) in enumerate(skf.split(X, y)):
        ...
        test_preds = bst.predict(xgb.DMatrix(
            test[feature_names]), num_iteration=iter_)
        preds += test_preds
    return preds / n_folds
...
def load_data(input_dir):
    ...
    for date_col in ['booking_date', 'checkin_date', 'checkout_date']:
        ...
        D['booking_in_advance'] = (
            D['checkin_date'] - D['booking_date']).dt.days
        ...
        D['season_holidayed_code'] = D['season_holidayed_code'].astype(
            np.uint8)
    ...
    agg_features_list = [
        ...
        (['memberid', 'resort_type_code', 'room_type_booked_code'],
         'reservation_id', 'size'),
        (['memberid', 'resort_type_code', 'room_type_booked_code'],
         'days_stayed', 'mean'),

        (['memberid', 'booking_date', 'checkin_date',
          'checkout_date'], 'reservation_id', 'size'),
        ...
        (['resort_id', 'checkin_date_mon', 'room_type_booked_code'],
         'reservation_id', 'size'),
        (['resort_id', 'main_product_code'], 'reservation_id', 'size'),
        (['resort_id', 'main_product_code', 'channel_code'],
         'reservation_id', 'size'),
        ...
        (['resort_id', 'state_code_residence',
          'season_holidayed_code'], 'reservation_id', 'size'),
        ...
        (['resort_id', 'checkin_date_year', 'checkin_date_mon'],
         'reservation_id', 'size'),
    ]
    ...
    D["booking_idx_memberid"] = D.groupby(
        "memberid")["reservation_id"].cumcount() + 1
    D['days_since_last_checkin'] = D.groupby(
        'memberid')['checkin_date'].diff().apply(lambda x: x.days)

    D.sort_values(["memberid", "resort_id", "checkin_date"],
                  inplace=True, ascending=True)
    D["booking_idx_memberid__resort_id"] = D.groupby(["memberid", "resort_id"])[
        "reservation_id"].cumcount() + 1
    D['days_since_last_checkin_resort'] = D.groupby(['memberid', 'resort_id'])[
        'checkin_date'].diff().apply(lambda x: x.days)

    D.sort_values(["memberid", "booking_date", "checkin_date"],
                  inplace=True, ascending=True)
    D["idx_continued_trip"] = D.groupby(["memberid", "booking_date", "checkin_date"])[
        "reservation_id"].cumcount() + 1

    D.sort_values(["memberid", "resort_id", "checkout_date",
                   "checkin_date"], inplace=True, ascending=True)
    D["idx_inplace_booking"] = D.groupby(["memberid", "resort_id", "checkout_date"])[
        "reservation_id"].cumcount() + 1

    D.sort_values(["memberid", "checkin_date"], inplace=True, ascending=True)
    D["previous_checkout"] = D.groupby(["memberid"])["checkout_date"].shift()
    D["diff_checkin_checkout"] = (
        D["checkin_date"] - D["previous_checkout"]).dt.days
    ...
    D['days_since_last_checkin_resort'] = D['days_since_last_checkin_resort'].astype(
        int)
    ...
    features = [col for col in D.columns if col not in [
        'reservation_id', 'amount_spent_per_room_night_scaled', 'memberid']]
...
if __name__ == '__main__':
    ...
    df = pd.DataFrame({"reservation_id": test_ids,
                       "amount_spent_per_room_night_scaled": final_preds})
